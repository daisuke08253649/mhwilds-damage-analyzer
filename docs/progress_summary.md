...
- Gemini API消費量モニタリング設定：未着手（`gemini-3.1-flash-lite`は有料ティアありのため、無料枠超過時の課金リスクを踏まえて優先度高め）

### 👉 次のアクション（再開時の起点）

1. Gemini API消費量モニタリング設定に着手する（`gemini-3.1-flash-lite`への切り替えに伴うコスト超過リスクの監視）
2. 50分動画でのパフォーマンス・メモリ使用量を確認する（今回の短尺クリップでは338MBだったが、長尺・高頻度ヒットのケースでの挙動は未検証）
